# Anima ControlNet-LLLite custom training script (text embedding steering & timestep clamping)
...
from networks.control_net_lllite_anima_custom import (
    DecoupledControlNetLLLiteDiT as ControlNetLLLiteDiT,
    DecoupledAnimaControlNetLLLiteWrapper as AnimaControlNetLLLiteWrapper,
    save_lllite_model,
    load_lllite_weights,
    LLLITE_ARCH_VERSION,
    LLLITE_PRESETS,
    LLLITE_ATOMIC_SPECIFIERS,
)
...
# --- [CUSTOM DECOUPLED MONKEY-PATCH] ---
# We dynamically replace train_util.ControlNetDataset.__getitem__ to bypass original resolution matching assertions
# and enforce a size-agnostic 1MP limit (preserving original aspect ratio and multiples of 16).
def custom_controlnet_dataset_getitem(self, index):
...
train_util.ControlNetDataset.__getitem__ = custom_controlnet_dataset_getitem
# ---------------------------------------


def _load_control_image(path: str, device, dtype) -> torch.Tensor:
    """Load a control image, limit it to at most 1MP, keep original aspect ratio (stride 16 aligned), and return (1, 3, H, W) in [-1, 1]."""
...
def _make_lllite_sample_hooks(args, lllite, dit_dtype):
    ...
    def on_prompt_start(prompt_dict: dict, accelerator):
        ...
        # Load size-agnostic 1MP aligned control image preserving aspect ratio
        cond_image = _load_control_image(ci_path, accelerator.device, dit_dtype)
...
def add_anima_lllite_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="which block indices to attach LLLite modules to (comma-separated list and/or ranges, e.g., '10-20' or '5,6,7')",
    )
...
def custom_get_noisy_model_input_and_timesteps(
    ...
    if args.timestep_sampling == "uniform" or args.timestep_sampling == "sigmoid":
        ...
        # --- [CUSTOM CHANGE] Apply hard scaling & cutoff ---
        ...
        sigmas = sigmas * args.sigmoid_scale  # larger scale for more uniform sampling
        sigmas = sigmas.sigmoid()
        mu = flux_train_utils.get_lin_function(y1=0.5, y2=1.15)((h // 2) * (w // 2))  # we are pre-packed so must adjust for packed size
        ...
        # Sample a random timestep for each image using standard SD3 logic
...
def train(args):
    ...
    if args.dataset_class is not None:
        ...
        if args.style_dataset:
            # For style dataset, we don't require conditioning_data_dir in TOML
            sanitizer = ConfigSanitizer(True, False, False, True)
        else:
            sanitizer = ConfigSanitizer(False, False, True, True)
        ...
        if args.style_dataset:
            logger.info("Using style-based custom ControlNet dataset")
            datasets = []
            ...
            # Prepare the dataset: make buckets and set seed
    ...
    # Build LLLite (Decoupled Cross-Attention ControlNet)
    logger.info("Building ControlNet Decoupled Cross-Attention LLLite (Anima)...")
    ...
    accelerator.print("running training (Anima Decoupled Cross-Attention ControlNet-LLLite)")
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    # --- [CUSTOM CHANGE] Add hard-clamping limits ---
    parser.add_argument(
        "--timestep_min",
        type=float,
        default=0.2,
    ...
    parser.add_argument(
        "--style_dataset",
        action="store_true",
        help="Use custom style-based ControlNet dataset where conditioning images are randomly sampled from the same style folder.",
    )
    # -------------------------------------------------
